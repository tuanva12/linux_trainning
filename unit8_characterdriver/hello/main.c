...
struct device *my_dev;  // đại diện cho device
struct cdev my_cdevice;    // đại diện cho driver
...
static int __init func_init(void)
{
    ...
    /* Create cdev struct */
    cdev_init(&my_cdevice, &fops);

    my_cdevice.owner = THIS_MODULE;
    /* Adding character device to the system */
    cdev_add(&my_cdevice, dev_num, 1);
